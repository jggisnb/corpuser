...
from helper.function import random_d, random_w_d, random_w_d_, rawgencount
...
class markTextEdit(QTextEdit):
    ...
    def mark_with_single_info(self,info,textCursor):
        colorFormat = QTextCharFormat()
        colorFormat.setForeground(QBrush(QColor(info["color"])))
        ...
        if markinfo:
            for k, v in markinfo.items():
                if int(self.__shutcut_map[v["shutcut"]]) == self.__key_press:
                    self.mark_with_single_info(v,textCursor)
                    return
    ...
    def refresh_text_selected(self,regexs):
        wastecolor = None
        markinfo = self.check_legitimacy()
        if markinfo and "waste" in markinfo:
            wastecolor = markinfo["waste"]["color"].lower()
        ...
        for reg,color in regexs:
            ...
            while (not hightLight.isNull() and not hightLight.atEnd()):
                ...
                colorFormat = QTextCharFormat()
                if wastecolor and color.lower() == wastecolor:
                    colorFormat.setForeground(QBrush(QColor("#ffffff")))
                else:
    ...
    def render_text(self,text,regexs):
        self.setTextThread = uiThread((text,regexs))
    ...
    def __setText_and_render_sgl(self,args):
        try:
            ...
            if type(e) == KeyError:
                return self.error_emit.emit(f"缺少标注({str(e)}),请新建标注。")
            self.error_emit.emit(f"{type(e)}:{str(e)}")
...
from ..someview import frameview, clicked_frame


class shapeView(frameview):
    clicked_emit = pyqtSignal(str)
    def __col_frame_click_sgl(self):
        text = self.mark_edit.text()
        if text and len(text):
            self.clicked_emit.emit(text)

    def __init__(self,click_sgl,cur_idx=0):
        super(shapeView, self).__init__()
        self.clicked_emit.connect(click_sgl)
        layout = QVBoxLayout()
        layout0 = QHBoxLayout()
        layout1 = QHBoxLayout()
        layout2 = QHBoxLayout()
        ...
        self.__col_frame = clicked_frame()
        self.__col_frame.setMinimumSize(50,20)
        self.__col_frame.clicked_emit.connect(self.__col_frame_click_sgl)
        # self.__col_frame.setStyleSheet("""QWidget{background-color:}""")
        self.colort_text = "#d7686d"
        self.__col_frame.color = self.colort_text
        # pal = QPalette()
        # pal.setColor(QPalette.Background, QColor(self.colort_text))
        # self.__col_frame.setPalette(pal)
        self.__col_frame.setStyleSheet('background-color: %s' % self.colort_text)
        ...
        self.checkbox.setMaximumWidth(50)
...
class click_frame(QFrame):
    ...
    def __init__(self,color_str):
        ...
        # self.setStyleSheet(f"QWidget{{border:none;}}")
        self.setAutoFillBackground(True)
        self.__color_str = color_str
        pal = QPalette()
        pal.setColor(QPalette.Background,QColor(self.__color_str))
        self.setPalette(pal)
        self.is_click = False
...
class regexDialog(QDialog):
    ...
    change_config_emit = pyqtSignal()
    def __init__(self,*args,**kwargs):
        super(regexDialog, self).__init__()
        keys = kwargs.keys()
        self.__color_selected = None
        if "color_selected" in keys:
            self.__color_selected = kwargs["color_selected"]

        self.__default_regex = None
        if "default_regex" in keys:
            self.__default_regex = kwargs["default_regex"]

        self.__item = None
        if "widget" in keys:
            self.__item:view.listcell = kwargs["widget"]
    ...
    def __set_default_selected(self):
        for f in self.__frames:
            if f.get_color() == self.__color_selected:
                f.set_is_click(True)
                break
        self.regex_edit.setText(self.__default_regex)
    ...
    def execute(self,colors):
        ...
        # self.regex_edit.setMinimumHeight(50)
        ...
        # layout0.addLayout(self.framelayout)
        ...
        #         del frame
        #     self.__addColorFrame(colors)
        if self.__color_selected != None:
            self.__set_default_selected()
        self.exec_()
    ...
    def __confirm_sgl(self):
        ...
        try:
            ...
            return QMessageBox.warning(self, "警告", "正则表达式不合法，请修正。")
        ...
        for f in self.__frames:
            if f.is_click:
                color = f.get_color()
                if self.__item:
                    self.__item.edit.setText(text)
                    self.__item.set_color(color)
                    self.change_config_emit.emit()
                else:
                    self.add_color_emit.emit(color, text)
                self.close()
                return 1
...
class view(pkg_base_view):
        ...
        for mi in items:
            ...
            if not len(mark_value):
                if showMessage:
                    ...
                    return False,None
            ischeked = mi.checkbox.isChecked()
            ...
            self.markinfo[mark_value]["ischecked"] = ischeked
        return True,None

    def __regexDialog(self,*arg,**kwargs):
        _regexDialog = regexDialog(*arg,**kwargs)
        _regexDialog.add_color_emit.connect(self.__add_listcell)
        _regexDialog.change_config_emit.connect(self.__insert_part_sgl)
        return _regexDialog

    def __init_ui(self):
        self.regex_dialog = self.__regexDialog()
        layout0 = QHBoxLayout()
        ...
        self.textEdit.error_emit.connect(self.__edit_error_sgl)
        ...
        self.listWgt.itemDoubleClicked.connect(self.__re_edit_listcell)
        ...
        self.next_btn = QPushButton(text="导出(q)")
        self.next_btn.clicked.connect(self.__next_sentence)
        self.next_btn.setShortcut("alt+q")
        self.recall_btn = QPushButton(text="返回(w)")
        self.recall_btn.clicked.connect(self.__recall_sentence)
        self.recall_btn.setShortcut("alt+w")
        self.skip_btn = QPushButton(text="跳过(e)")
        self.skip_btn.clicked.connect(self.__skip_sentence)
        self.skip_btn.setShortcut("alt+e")
        ...
        layout3.addStretch(1)
        layout3.addWidget(self.skip_btn)
        layout3.addStretch(1)
        ...
        layout6 = QVBoxLayout()
        layout8 = QHBoxLayout()
        self.countLabel = QLabel(f"完成数量:0")
        self.exportCountLabel = QLabel(f"导出数量:0")
        layout8.addWidget(self.countLabel)
        layout8.addStretch(1)
        layout8.addWidget(self.exportCountLabel)
        layout8.addStretch(3)
        ...
        self.mark_layout.addWidget(shapeView(self.__shapeView_item_clicked))
    ...
    def __shapeView_item_clicked(self,text):
        legitimacy = self.check_legitimacy()
        if legitimacy[0]:
            textCursor = self.textEdit.textCursor()
            self.textEdit.mark_with_single_info(self.markinfo[text],textCursor)

    #---------------QPushButton---------------#
    def __add_mark_sgl(self):
        self.mark_layout.addWidget(shapeView(self.__shapeView_item_clicked,self.mark_layout.getLen()))
    ...
    def __skip_sentence(self):
        self.__inhibit_control()
        self.skip_thread = uiThread()
        self.skip_thread.task_no_args_emit.connect(self.__skip_sentece_sgl)
        self.skip_thread.start()
        self.skip_thread.finished.connect(self.onlogthread_finished)

    def __get_html_pure_text(self,html):
        ...
        for ft in full_tags:
            tags = ft.contents
            for tag in tags:
                brower_text += str(tag.string)
        return brower_text

    def __skip_sentece_sgl(self):
        try:
            if self._line_count > 0:
                html = self.textEdit.document().toHtml()
                b4 = BeautifulSoup(html, "html.parser")
                full_tags = b4.find_all("p")
                brower_text = "<p style=\"color:#ffffff;font-size:14px;background-color:black;\"><span style=\"color:yellow;\">跳过:</span>"
                for ft in full_tags:
                    tags = ft.contents
                    for tag in tags:
                        brower_text += str(tag)
    ...
    def __next_sentece_sgl(self):
        try:
            ...
            if self._line_count > 0:
                ...
                brower_text_log = "<p style=\"color:#ffffff;font-size:14px;background-color:black;\"><span style=\"color:#65CB64;\">插入:</span>"
                # pure_texts = []
                for ft in full_tags:
                    tags = ft.contents
                    for tag in tags:
                        ...
                        if "style" in str(tag):
                            ...
                            # if class_ != "O" and string not in pure_texts:
                            #     pure_texts.append(string)
                        else:
                            color = "#ffffff"
                            string = str(tag)

                        brower_text += f"<span class=\"{class_}\" style=\"color:{color};\">"
                        brower_text_log += f"<span class=\"{class_}\" style=\"color:{color};\">"
                        ...
                        brower_text += "</span>"
                        brower_text_log += "</span>"
                brower_text += "</p>"
                brower_text_log += "</p>"
                self.textBrowser.append(brower_text_log)
                ...
                self.__count += 1
                self.__setLabelCount()
                self.__setExportLabelCount()
                self._operateArray.append(1)
                if self._line_count > 0:
                    text = self._file_content[self._total_count-self._line_count]
                    self.__setText_and_selected(text)
                    return
    ...
    def __recall_sentece_sgl(self):
        try:
            ...
            if not os.path.exists(export_edit) and not os.path.isdir(export_edit):
                return QMessageBox.critical(self, "错误", f"导出目录不存在或不是一个文件夹:{export_edit}")
            export_edit = os.path.join(export_edit, f"export{self.__t1}.txt")
            if len(self._operateArray):
                current_text = self._file_content[self._total_count - self._line_count]
                lastOperate = self._operateArray.pop(-1)
                pure_text = self.__get_html_pure_text(current_text)
                if lastOperate == 0:
                    if self.__count > 0:
                        self.__count -= 1
    ...
    #---------------QListWidget---------------#
    def __re_edit_listcell(self,item: QListWidgetItem):
        ...
        if isinstance(cell,self.listcell):
            ...
            if legitimacy[0]:
                ...
                self.regex_dialog = self.__regexDialog(color_selected=color,default_regex=reg,widget=cell)
    ...
    class listcell(QWidget):
        ...
        def __init__(self, item,regex_str:str, color_str):
            ...
            self.edit.setMaximumWidth(300)
            self.edit.setToolTip(regex_str)
            thLayout = QHBoxLayout()
            self.color_frame = QFrame()
            self.color_frame.setMaximumSize(50,20)
            self.color_frame.setStyleSheet(f"QWidget{{background-color:{color_str};}}")
            self.__color_str = color_str
            sub_btn = QPushButton()
            ...
            thLayout.addWidget(self.color_frame)
        ...
        def set_color(self,color):
            self.__color_str = color
            self.color_frame.setStyleSheet(f"QWidget{{background-color:{self.__color_str};}}")
    ...
    def __setLabelCount(self):
        self.countLabel.setText(f"完成数量:{self.__count}")
    def __setExportLabelCount(self):
        self.exportCountLabel.setText(f"导出数量:{rawgencount(os.path.join(self.export_edit.text(),f'export{self.__t1}.txt'))}")

    def __edit_error_sgl(self,error:str):
    ...
    def __setText_and_selected(self,text):
        if not text.startswith("<p>"):
            ...
            self.textEdit.render_text(text,self.get_regexs())
    ...
    def addLog(self,log,flag):
        if flag == 1:
            ...
            self.textBrowser.append(f"<p style='color:yellow;font-size:14px;'>{log}</p>")
        import time
        time.sleep(0.01)
